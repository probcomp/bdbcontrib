...
def test_is_plotting_command():
    ...
    assert general_utils.is_plotting_command(cmd1)
    assert general_utils.is_plotting_command(cmd2)
    assert general_utils.is_plotting_command(cmd3)
    assert not general_utils.is_plotting_command(cmd4)
    assert not general_utils.is_plotting_command(cmd5)
    assert not general_utils.is_plotting_command(cmd6)
...
def test_clean_cmd_filename():
    ...
    cmd1_ud, _ = general_utils.clean_cmd_filename(cmd1, fignum, output_dir)
    assert cmd1_ud == cmd1_expected
    with pytest.raises(ValueError):
        general_utils.clean_cmd_filename(cmd2, fignum, output_dir)
    with pytest.raises(ValueError):
        general_utils.clean_cmd_filename(cmd1_expected, fignum, output_dir)


def test_is_comment():
    assert not general_utils.is_comment('.heatmap ESTIMATE PAIRWISE FOO;')
    assert general_utils.is_comment('-- is a comment')
    assert general_utils.is_comment('--is a comment')
    assert general_utils.is_comment('---is a comment')

    # XXX: There is no support for inline comments in BQL scripts
    assert not general_utils.is_comment(' -- not a comment')
    assert not general_utils.is_comment('.show SELECT a, b FROM t;'
        ' --no-contour')


def test_is_dot_command():
    assert not general_utils.is_dot_command('-- .show SELECT a, b FROM t;')
    assert not general_utils.is_dot_command('SELECT ".show" FROM t;')
    assert general_utils.is_dot_command('.show SELECT a, b FROM t;')
